...
$expiry_time = strtotime('-24 hours'); // Calculate the timestamp for 24 hours ago

// 1. Read the JSON file content
if (file_exists($json_file)) {
    $json_content = file_get_contents($json_file);
    $posts = json_decode($json_content, true); // Decode as associative array
} else {
    die("Error: posts.json not found.");
}

// Check if decoding was successful and $posts is an array
if (!is_array($posts)) {
    die("Error: Invalid JSON data in posts.json.");
}

// 2. Filter the posts, keeping only those newer than 24 hours
$filtered_posts = array_filter($posts, function($post) use ($expiry_time) {
    // Assuming each post has a 'timestamp' key in Unix epoch format
    // If your timestamp is in a different format (e.g., "YYYY-MM-DD H:i:s"), use strtotime() to convert it
    $post_timestamp = is_numeric($post['timestamp']) ? (int)$post['timestamp'] : strtotime($post['timestamp']);
    
    // Keep the post if its timestamp is greater than the expiry time
    return $post_timestamp >= $expiry_time;
});

// Use array_values to re-index the array numerically after filtering
$filtered_posts = array_values($filtered_posts);

// 3. Encode the filtered data back into JSON format
$new_json_content = json_encode($filtered_posts, JSON_PRETTY_PRINT);

// 4. Write the new JSON content back to the file
if (file_put_contents($json_file, $new_json_content) !== false) {
    echo '<img src="https://i.imgur.com/YR2ha2i.png" alt="Description of image" / style="width:20px; height:20px;">';
    echo "<st><i>Old posts have been removed and posts.json updated successfully at " . date('Y-m-d H:i:s') . "</i></st>";
} else {
    echo "Error: Unable to write to posts.json.";
}
...
></div>
 ...
 <button onclick="window.location.href='/boards/b/catalog.php'">Enter catalog view</button>
    
 <button onclick="window.location.href='/index.php'">Return to home page</button>
    
     <button onclick="window.location.href='/boards/b/archive.php'">View thread archives</button>
